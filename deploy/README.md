...
- `sms_forwarder/sms_forwarder.py`: polls ModemManager for newly received SMS and forwards them through Apprise or native enterprise channels.
- `sms_forwarder/sms-forwarder.service`: systemd unit for the SMS forwarder.
- `shared/notification_utils.py`: shared notification target parsing, Apprise delivery, and native sender helpers.
- `sms_forwarder/sms-forwarder.conf.example`: example configuration for notification targets.
